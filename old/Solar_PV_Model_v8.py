...
def run_simulation(fALPHA, fBETA, fN_of_firms, finitial_tc_upper, finitial_tc_lower, finitial_cc_upper, finitial_cc_lower, finitial_projects_total, finitial_lc_mean, finitial_lc_variance, fOKshare, fSIM_TIME, fRANDOM_SEED, fNUMBER_OF_REPETITIONS, fresponsivities, fRANDOMIZE):
   ...
   #Parameter_in_the_smiulation = function_input
    ...
    for rep in range (RANDOM_SEED, RANDOM_SEED + NUMBER_OF_REPETITIONS): #the +1 produces 1 repetition
        ...
        for j in responsivities:
            ...
            for i in range(N_of_type1): #This starts us with the number of firms
            ...
            for i in range(N_of_type1+1, N_of_type2-N_of_type1): #This starts us with the number of firms
                env.process(firm(env, 'Firm %d' % i, 0, 0, random.uniform(initial_tc_lower, initial_tc_upper), random.uniform(initial_cc_lower, initial_cc_upper), projects_dictionary.get(i), projects_dictionary.get(i), 0, 0, 0))                    
...
def firm(env, name, fBNDES, fbanks, tc, cc, projects_total, projects_remaining, projects_financed_BNDES, projects_financed_bank, projects_failed_BNDES):
    ...
    while True:
        ...
        if env.now>0:
            ...
            name_t.append(name)
            SIM_t.append(SIM)
            index_t.append(index_t[-1] + LC*(projects_financed_BNDES-projects_financed_BNDES_tminus1))
